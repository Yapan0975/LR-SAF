...
def main(epochs=15, lr=1e-3, seed=42, afm_ckpt=None, out_head=None, out_json=None):
    ...
    def eval_val():
        ...
        for v in val_data:
            if v is None:
                sap_list.append(0.0); continue
            with torch.no_grad():
                scores = head(v['feats']).cpu().numpy().squeeze()
            aps = s_ap(v['kept'][:, :4], scores, v['gt'], v['H_orig'], v['W_orig'],
                       thresholds=(5, 10, 15))
            sap_list.append(aps[10])
        head.train()
        return float(np.mean(sap_list))
    ...
    for ep in range(epochs):
        ...
        for batch in train_data:
            optim.zero_grad()
            logits = head(batch['feats']).squeeze(-1)
            loss = F.binary_cross_entropy_with_logits(logits, batch['tgt'])
            loss.backward(); optim.step()
            ep_losses.append(loss.item())
        L = float(np.mean(ep_losses)) if ep_losses else float('nan')
        sap10 = eval_val()
        log['train_loss'].append(L); log['val_sAP10'].append(sap10)
        print(f"ep {ep:02d}: loss={L:.4f}  val sAP-10={sap10:.4f}")
        if sap10 > best_sap:
            best_sap = sap10
            if out_head:
                os.makedirs(os.path.dirname(out_head) or '.', exist_ok=True)
                torch.save({'head': head.state_dict(), 'ep': ep, 'val_sAP10': sap10},
                           out_head)
    print(f"\nbest val sAP-10 (AFM+geom-head): {best_sap:.4f}")
    if out_json:
        with open(out_json, 'w') as fh:
            json.dump({'best_sAP10': best_sap, 'log': log,
                       'afm_ckpt': afm_ckpt}, fh, indent=2)
